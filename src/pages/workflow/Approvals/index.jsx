...
export default class MyProcess extends Component {
  ...
  getAccessToken = async () => {
    ...
    if (res.data.data && res.data.data.success && res.data.data.AccessToken) {
      return res.data.data.AccessToken;
    }
  };
  // 获取BPM待办事项
  getBPMTodoList = async () => {
    const accessToken = await this.getAccessToken();
    const resp = await fetch(API.GET_TASK_LIST, {
      method: 'POST',
      headers: {
    ...
    this.setState({
      list: this.state.list.concat(result.data.TaskList),
      isLoading: false,
      loading: false,
  ...
  // 获取BPM历史事项
  getBPMHistoryList = async () => {
    const accessToken = await this.getAccessToken();
    const resp = await fetch(API.GET_HISTORY_LIST, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: accessToken,
      },
      body: JSON.stringify({ Page: 1, PageSize: 100, Year: 2025 }),
    });
    if (!resp.ok) {
      throw new Error('Network response was not ok');
    }
    ...
    return (
      <div className="header card">
        <div className="valignWrapper flex title">
          <Icon icon="task_alt" />
          <span className="bold">{_l('审批中心')}</span>
        </div>
